...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'Файл WADL недоступен. Перезапустите приложение RESTful и обновите эту страницу или укажите URL-адрес. Причина может быть в том, что данная реализация REST не предоставляет файл WADL автоматически (например, JBoss RESTEasy). В этом случае необходимо предварительно создать его вручную или переключиться на библиотеки Jersey.';
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', 'span class=bld>Страница справки</span><br/><br/><p>Файл WADL недоступен. Перезапустите приложение RESTful и обновите эту страницу или укажите URL-адрес. Причина может быть в том, что данная реализация REST не предоставляет файл WADL автоматически (например, JBoss RESTEasy). В этом случае необходимо предварительно создать его вручную или переключиться на библиотеки Jersey.</p><p>Если ошибка не будет устранена, и если вы используете Firefox с подключаемым модулем Firebug, <br/>необходимо отключить Firebug для локальных файлов. Для этого в строке меню Firefox установите флажок <br/>Инструменты > Firebug > Отключить Firebug для локальных файлов</p>');
        }            
    },
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if ( (rtext== undefined || rtext == '' ) && (method=='POST' || method=='PUT' || method=='DELETE')){
                  return 'В ответе отсутствует содержимое';
              }
